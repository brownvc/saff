...
if __name__ == "__main__":
    ...
    for scene in tqdm(scenes):
        os.makedirs(os.path.join(out_folder, scene), exist_ok=True)
        # find the background colors based on first frame
        gt_img = cv2.imread(os.path.join(args.gt_folder, scene, "training_ 0.png"), cv2.IMREAD_GRAYSCALE)
        raw_img = cv2.imread(os.path.join(args.raw_folder, scene, "0.png"))
        ...
        for idx in range(60):
            raw_img = cv2.imread(os.path.join(args.raw_folder, scene, f"{idx}.png"))
            for color in background:
                region = (raw_img[..., 0:1] == color[0]) & (raw_img[..., 1:2] == color[1])& (raw_img[..., 2:3] == color[2])            
                raw_img[region[..., 0]] *= 0
            cv2.imwrite(os.path.join(out_folder, scene, f"{idx}.png"), raw_img)
