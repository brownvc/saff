...
if __name__ == "__main__":
    ...
    while os.path.exists(os.path.join(root_dir, f"{image_id}.png")):
        ...
        old_img = copy.deepcopy(img)
        label_im = label(img, connectivity=2)
        imsave(os.path.join(out_dir,  f"{image_id}_label.png"), label_im)
        #regions = regionprops(label_im)
        list_of_index = []
        ...
        to_collapse = ~np.isin(label_im, list_of_index) 
        imsave(os.path.join(out_dir,  f"{image_id}_small.png"), to_collapse)
        ...
        tmp[to_collapse, :] *= 0 
        #assert False, imsave("test.png", tmp)
        cv2.imwrite(os.path.join(out_dir, f"{image_id}.png"), tmp)
        image_id += 1
